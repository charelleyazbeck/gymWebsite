                    ...
                    <div class="cart-container">
                        <div class="container">
                            <div class="section-title text-center">
                                <h2 class="store-word">This Order Contains:</h2>
                            </div>

                            <div class="cart-container" >
                                <div class="cartlistproducts">
                                    <table class="itemstable">
                                        <tr><td>Item Name</td><td>Image</td><td>Quantity</td><td>Unit price</td><td>Total price</td></tr>
                                        <?php
                                        $query3 = "SELECT * FROM orders WHERE o_ID='$o_id'";
                                        $result2 = $con->query($query3);
                                        if ($result2->num_rows > 0) {
                                            $row3 = $result2->fetch_assoc();
                                            $orderprice = $row3['oAmmount'];
                                            $query = "SELECT * FROM orderitems WHERE o_ID='$o_id'";
                                            $res = $con->query($query);
                                            if ($res) {
                                                if ($res->num_rows == 0) {
                                                    ...
                                                    while ($row = $res->fetch_assoc()) {
                                                        $query2 = "SELECT * FROM items WHERE i_ID='$row[i_ID]'";
                        ...
                        <td>
                        <span class='quantity'>$row[itemQuantity]</span>
                        </td>
                        <td class='unit-price'>$price $</td>
                        <td class='total-price'>" . (int) $row['priceAtPurchase'] * (int) $row['itemQuantity'] . " $</td>                        
                        </tr>";
                                                    }
                                                }
                                            }
                                            ...
                                            $orderprice = 0;
                                            echo "<tr><td colspan='5'>No Items In this Order</td></tr>";
                                        }
                                        ?>                        
                                        <tr>
                                            <td colspan="4">Order Price:</td>
                                            <td id="order-price" colspan="2"><?php echo $orderprice . "$" ?></td>
                                        </tr>
                                    </table>
                                </div>
                            </div>
