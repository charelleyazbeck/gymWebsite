...
<script src="js/main.js"></script>
        <form>
            <table border="2" style="margin: 3px;" class="viewtable"><thead>
                <?php
                $query = "SELECT * FROM orders WHERE oStatus = 'waiting'";
                $res = $con->query($query);
                if ($res) {
                    echo "<tr><th>Order ID</th><th>Total Price</th><th>Show Product</th><th>Complete Order</th><th>Client ID</th></tr></thead><tbody>";
                    if ($res->num_rows > 0) {
                        while ($row = $res->fetch_assoc()) {
                            echo "<tr>";
                            echo "<td>$row[o_ID]</td>
                                <td>$row[oAmmount]</td>
                <td><a href='showproducts.php?order_id=$row[o_ID]'><img src='showicon.png' width='45px' height='40px' ></a></td>
                <td><a href='completeorder.php?order_id=$row[o_ID]'><img src='assignicon.png' width='45px' height='40px'></a></td>";
                            echo "<td>$row[c_ID]</td>";
                            echo "</tr>";
                        }
                        
                    }else{
                        echo "<tr><td colspan='5'>No current orders</td></tr>";
                    }echo "</tbody>";
                }
                ?>
            </table>
