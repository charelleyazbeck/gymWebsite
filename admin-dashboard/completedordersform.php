...
<form>
            <table border="2" style="margin: 3px;" class="viewtable"><thead>
                <?php
                $query = "SELECT * FROM orders WHERE oStatus = 'completed'";
                $res = $con->query($query);
                if ($res) {echo "<tr><th>Order ID</th><th>Order Price</th><th>Show Product</th><th>Client ID</th></tr></thead><tbody>";
                    if ($res->num_rows > 0) {
                ...
                <td><a href='showproducts.php?order_id=$row[o_ID]'><img src='showicon.png' width='45px' height='40px' ></a></td>";
                        ...
                        echo "<tr><td colspan='4'>No current orders</td></tr>";
